...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const productsUrl = `http://localhost:3001/json/cats_products/${selectedCategoryID}.json`;
  ...
  fetch(productsUrl)
    ...
    .then((data) => {
      ...
      const sortDescRelevanceButton =
        document.getElementById("sortDescRelevance");
      ...
      const performSearch = (searchText, productsToShow) => {
        ...
        if (sortAscPriceButton.checked) {
          sortedProducts = searchResults
            .slice()
            .sort((a, b) => parseFloat(a.cost) - parseFloat(b.cost)); // dicho metodo .sort devuelve un numero negativo, cero o positivo, segun si el costo de a es menor, igual o mayor que el costo de b. Ordena los productos de menor a mayor
        } else if (sortDescPriceButton.checked) {
          sortedProducts = searchResults
            .slice()
            .sort((a, b) => parseFloat(b.cost) - parseFloat(a.cost)); // Misma logica que el boton de ascendente pero invertida para que sea descendente, de Mayor a menor.
        } else if (sortDescRelevanceButton.checked) {
          sortedProducts = searchResults
            .slice()
            .sort((a, b) => parseInt(b.soldCount) - parseInt(a.soldCount)); // Misma logica que los otros dos botones, pero este toma como parametro la cantidad de articulo que tenga y no el precio.
